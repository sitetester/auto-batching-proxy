...
#[cfg(test)]
// convenience wrapper to run ALL tests from IDE
mod embed_functionality_tests {
    ...
    mod using_app_config_defaults_test {
        use super::*;

        async fn with_num_inputs_using_defaults(num: usize) {
        ...
        #[tokio::test]
        async fn test_embed_endpoint_success_with_single_input_using_defaults() {
            with_num_inputs_using_defaults(1).await
        }

        #[tokio::test]
        async fn test_embed_endpoint_success_with_multiple_inputs_using_defaults() {
            with_num_inputs_using_defaults(3).await
        }
    }
    ...
    mod max_batch_size_tests {
        ...
        #[tokio::test]
        async fn test_max_batch_size_should_process_first_with_single_input_per_request() {
            max_batch_size_should_process_first_with_num_inputs_per_request(1).await;
        }

        #[tokio::test]
        async fn test_max_batch_size_should_process_first_with_multiple_inputs_per_request() {
            max_batch_size_should_process_first_with_num_inputs_per_request(3).await;
        }

        #[tokio::test]
        async fn test_max_batch_size_while_exceeding_max_inference_inputs() {
            let config = AppConfig {
    ...
    mod max_wait_time_ms_tests {
        ...
        #[tokio::test]
        async fn test_max_wait_time_ms_should_process_first_with_single_input_per_request() {
            max_wait_time_ms_should_process_first_with_num_inputs_per_request(1).await;
        }

        #[tokio::test]
        async fn test_max_wait_time_ms_should_process_first_with_multiple_input_per_request() {
            max_wait_time_ms_should_process_first_with_num_inputs_per_request(5).await;
        }

        #[tokio::test]
        async fn test_max_wait_time_ms_while_exceeding_max_inference_inputs() {
            let config = AppConfig {
                include_batch_info: true,
                max_batch_size: 100,
                max_wait_time_ms: 500, // smaller value
